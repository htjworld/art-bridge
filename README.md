...
- 전국 무료 공연 10개 우선 수집
- 무료 공연이 5개 미만이면 저렴한 유료 공연으로 10개 채움
- sidoCode로 지역 필터링 가능
- ⚠️ 중요: startDate/endDate 파라미터는 무시됩니다 (항상 오늘~30일 고정)

**입력 파라미터:**
- `genreCode`: 장르 코드
- `startDate`: 공연 시작일 (YYYYMMDD) - 기능을 위해 무시됨
- `endDate`: 공연 종료일 (YYYYMMDD) - 기능을 위해 무시됨
- `sidoCode`: 시도 코드 [선택]
- `limit`: 무시됨 (기본: 10) [선택] - 기능을 위해 무시됨
...
### 3. 다중 세션 지원
- 세션별 SSE transport 관리로 여러 클라이언트 동시 접속 지원
- 30분 자동 타임아웃 및 세션 정리
