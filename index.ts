...
async function handleMcpRequest(req: Request, res: Response) {
  ...
  try {
    ...
    // ✅ 올바른 메서드: handleRequest(req, res, req.body)
    await transport.handleRequest(req, res, req.body);
    
    console.error('MCP request completed:', req.body?.method);
  } catch (error) {
...
// ✅ Streamable HTTP 엔드포인트 (Stateless 모드) - 수정됨
async function handleMcpRequest(req: Request, res: Response) {
  console.error(`${req.method} /mcp - MCP request`);
  ...
  try {
    ...
    transport = new StreamableHTTPServerTransport({
      sessionIdGenerator: () => undefined  // Stateless: session 없음
    });
